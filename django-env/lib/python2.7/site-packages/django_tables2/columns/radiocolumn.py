...
class RadioColumn(Column):
    '''
    A subclass of `.Column` that renders as a radio form input.
    ...
    used as the value for the radio, i.e. ``<input type="radio"
    value="..." />``
    ...
    Arguments:
        ...
        checked (`~.Accessor`, bool, callable): Allow rendering the radio as
            checked. If it resolves to a truthy value, the radio will be
            rendered as checked.

    .. note::

        You might expect that you could select multiple radios in the
    ...
    def __init__(self, attrs=None, checked=None, **extra):
        ...
        kwargs.update(extra)
        super(RadioColumn, self).__init__(**kwargs)
